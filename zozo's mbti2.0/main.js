...
var app = http.createServer(function(request,response){
    ...
    if(pathname === '/'){//pathname이 /이다=주소뒤에 아무것도 붙지않은 최상위 경로를 뜻함.
      if(queryData.id === undefined){//쿼리 아이디 값이 없다면
        db.query(`SELECT * FROM new_table`,function(error,topics){//2번째 인자=첫번째 sql문이 실행 된 후에 서버가 응답된 결과를 우리가 처리할 수 있도록 callback function을 주게 되있다.
  ...
  db.query(`SELECT * FROM new_table WHERE mbtitype='${queryData.id}'`, function(error2,topic){//queryData.id는 현재 mbtitype이다.
      if(error2){
  ...
  db.query(`SELECT * FROM new_table WHERE id='${queryData.id}'`, function(error2,topic){//queryData.id는 현재 mbtitype이다.
    if(error2){
  throw error2;//topic값 못가져왔을 시 error가 null이 아니게 되고 그러면 throw error를 하고 nodejs가 다음 명령 안시키고 콘솔표현하며 즉시 app중지 시킴.
}
  ...
  request.on('end', function(){
      ...
      db.query(`select count(*) as jobcount from new_table where mbtitype = ? and jobtype =?`,
       [post.title, post.description],//위값,아래 값 들어감
       function(error, result){
          if(error){
            throw error;
          }
          db.query(`select count(*) as peoplecount from new_table where mbtitype = ? `,
          [post.title],//위값,아래 값 들어감
          function(error, people){
             if(error){
               throw error;
             }          var cal= (result[0].jobcount)/(people[0].peoplecount);
          var title = `본인의 성격 '${post.title}'을 가진 사람들 중 본인의 직업 '${post.description}'에 종사하는 비율은 '${cal}'% 입니다`;
          var description = '성격에 따른 직업 적성도를 체크하세요';
